MODULE STM32F4IWDG IN Micro;
	...
	*)
    ...
    PROCEDURE Update*;
	BEGIN
		SYSTEM.PUT(MCU.IWDGKR, SIGNED32(0AAAAH))
	END Update;
	...
	BEGIN
		...
		(* enable LSI *)
			...
			REPEAT UNTIL SYSTEM.BIT(MCU.RCCCSR, LSIRDY);

		(* setup IWDG *)

			(* enable write access *)
				SYSTEM.PUT(MCU.IWDGKR, SIGNED32(5555H));

			(* setup PR *)
				REPEAT UNTIL ~SYSTEM.BIT(MCU.IWDGSR, PVU);
				SYSTEM.PUT(MCU.IWDGPR, PR);

			(* setup RLR *)
				REPEAT UNTIL ~SYSTEM.BIT(MCU.IWDGSR, RVU);
				SYSTEM.PUT(MCU.IWDGRLR, RL);

		(* reload IWDG *)
			SYSTEM.PUT(MCU.IWDGKR, SIGNED32(0AAAAH));

		(* start IWDG *)
			SYSTEM.PUT(MCU.IWDGKR, SIGNED32(0CCCCH))
	END Init;
