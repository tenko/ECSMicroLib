...
CONST
    ...
    (* APB3 *)
        RTC* = ADDRESS(44007800H);
            RTC_TR*         = ADDRESS(RTC + 0);
            RTC_DR*         = ADDRESS(RTC + 4);
            RTC_SSR*        = ADDRESS(RTC + 8);
            RTC_ICSR*       = ADDRESS(RTC + 0CH);
            RTC_PRER*       = ADDRESS(RTC + 10H);
            RTC_WUTR*       = ADDRESS(RTC + 14H);
            RTC_CR*         = ADDRESS(RTC + 18H);
            RTC_WPR*        = ADDRESS(RTC + 24H);
            RTC_CALR*       = ADDRESS(RTC + 28H);
            RTC_PRIVCFGR*   = ADDRESS(RTC + 1CH);    
            RTC_SHIFTR*     = ADDRESS(RTC + 2CH);
            RTC_SCR*        = ADDRESS(RTC + 5CH);
    (* NVIC *)
    (* interrupt sources *)
        RTCInt*     = 2;
        EXTI0Int*   = 7;
