MODULE ARMv8MSTM32SysTick0 IN Micro;
(*
...
CONST ENABLE = 0;
BEGIN RETURN SYSTEM.BIT(ARMv8M.SYST_CSR, ENABLE)
END Enabled;
...
BEGIN
	...
	SYSTEM.PUT(ARMv8M.SYST_CSR, x + {ENABLE})
END Enable;
...
VAR
	x: INTEGER;
BEGIN
	SYSTEM.PUT(ARMv8M.SYST_CSR, SET32({})); (* disable SysTick *)
	freq := hz;
	tick := 0;
	flag := FALSE;
	...
	x := HCLK DIV freq - 1;
	ASSERT(x > 0);
	SYSTEM.PUT(ARMv8M.SYST_RVR, x);
	SYSTEM.PUT(ARMv8M.SYST_CVR, SIGNED32(0)); (* any write to current clears it *)
	SYSTEM.PUT(ARMv8M.SYST_CSR, SET32({ENABLE,TICKINT, CLKSOURCE})) (* enable timer with clock source of SYSCLOCK with interrupts *)
END Init;
