MODULE STM32F4Flash IN Micro;
	...
	*)
	...
	BEGIN
		IF SYSTEM.BIT(MCU.FLASHOPTCR, OPTLOCK) THEN
			SYSTEM.PUT(MCU.FLASHOPTKEYR, FLASHOPTKEY1);
			SYSTEM.PUT(MCU.FLASHOPTKEYR, FLASHOPTKEY2)
		END
	END OBUnLock;
    ...
    PROCEDURE OBLock*;
		VAR x: SET;
	BEGIN
		SYSTEM.GET(MCU.FLASHOPTCR, x);
		SYSTEM.PUT(MCU.FLASHOPTCR, x + {OPTLOCK})
	END OBLock;
	...
	BEGIN
		...
		res := WaitForLastOperation();
		IF res = complete THEN
			SYSTEM.PUT(MCU.FLASHOPTCR, x);

			SYSTEM.GET(MCU.FLASHOPTCR, x);
			SYSTEM.PUT(MCU.FLASHOPTCR, x + {OPTSTRT});
